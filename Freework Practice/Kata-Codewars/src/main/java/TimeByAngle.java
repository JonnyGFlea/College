 ...
 *
 * And because the local council has lost most of our tax money to a Nigerian email scam there are no funds to
 * fix the clock properly.
 *
 * Instead, they are asking for volunteer programmers to write some code that tell the time by only looking
 * at the remaining hour-hand!
 *
...
public class TimeByAngle {
    public static String whatTimeIsIt(final double angle) {
        String returnTime = "";
        double angleHourRemainder = angle%30;
        double hours = (angle-angleHourRemainder)/30;
        double angleMinRemainder = angleHourRemainder%5;
        double minutes = ((angleHourRemainder-angleMinRemainder)/5)/10;
        ...
        if(angle < 30){
            ...
            bigDecimalHourHolder = bigDecimalHourHolder.add(BigDecimal.valueOf(hours));
        }

        bigDecimalMinHolder = bigDecimalMinHolder.add(BigDecimal.valueOf(minutes));
        while(angleMinRemainder > 0.499999){
            angleMinRemainder -= 0.5;
    ...
    private static String ConvertToReturnFormat(BigDecimal bigDecimalHourHolder) {
        ...
        if(bigDecimalHourHolder.intValue() < 10){
            returnTime = "0" + String.valueOf(bigDecimalHourHolder) + "00";
        }
        else{
            returnTime = String.valueOf(bigDecimalHourHolder) + "00";
        }
