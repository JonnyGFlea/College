...
void RedBlackTree::changeColors(Node*& uncle, Node*& parent, Node*& grandparent){
  out << "Changing colors between uncle: " << uncle->data << " parent: " << parent->data << " grandparent: " << grandparent->data << endl;
...
Node* RedBlackTree::delVal(Node*& root, int data){
  if(root == NULL){
    return root;
  }
  if(data < root->data){
    return delVal(root->left, data);
  }
  if(data > root->data){
    return delVal(root->right, data);
  }
  ...
  root->data = tmp->data;
  return delVal(root->right, tmp->data);
}
  ...
  while(ptr != root && isRed(ptr) && isRed(ptr->parent)){
    ...
    if(parent == grandparent->left){
      Node* uncle = grandparent->right;
        if(isRed(uncle)){
          changeColors(uncle, parent, grandparent);
          ptr = grandparent;
        }
        else{
          if(ptr == parent->right){
...
void RedBlackTree::insert(int data){
  out << "Insertion #" << count++ << endl;
  Node* node = new Node(data);
  root = insert(root, node);
  fixInsertViolation(node);
}

void RedBlackTree::delNode(Node*& p){
  Node* node = delVal(root, p->data);
...
Node* RedBlackTree::getMin(){
  return getMin(root);
}

Node* RedBlackTree::getMax(){
  return getMax(root);
}
